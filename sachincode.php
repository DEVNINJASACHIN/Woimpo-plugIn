...
function license_key_security() {
 ...
 to validate license key: ' . $api_response->get_error_message());
    return;
  }
...
function woimpo_schedule_tasks() {
  ...
  if (!wp_next_scheduled('woimpo_import_woocommerce_products')) {
    wp_schedule_event(time(), 'daily', 'woimpo_import_woocommerce_products');
  }

  if (!wp_next_scheduled('woimpo_create_or_insert_categories')) {
    wp_schedule_event(time(), 'daily', 'woimpo_create_or_insert_categories');
  }
}
